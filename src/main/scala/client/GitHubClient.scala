...
object GitHubClient extends Http4sClientDsl[IO]{
   ...
   private val client= JavaNetClientBuilder[IO]
                                   .withReadTimeout(8.minutes).create
...
def getContributors(org_name:String): IO[List[Contributor]] = for{
          ...
          contributorsJson<- IO.parTraverseN(contributorsUrl.length*3)(contributorsUrl)(fetchContributorsJson(client,token,_).compile.toList ) // for each url in the list, we past that to the function and get a list of contributors for that repo
